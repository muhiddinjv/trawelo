...
const getApis = async (cityName) => {
  try {
    const geoData = await axios.get(`${GEO_URL}=${cityName}&maxRows=1&username=${process.env.GEO_USER}`);

    const { name, countryName, lat, lng } = geoData.data.geonames[0];

    const [weatherData, pixData1, pixData2] = await Promise.all([
      axios.get(`${WB_URL}&lat=${lat}&lon=${lng}&days=7&key=${process.env.WB_KEY}`),
      axios.get(`${PIX_URL}key=${process.env.PIX_KEY}&q=${name}&image_type=photo&orientation=horizontal&category=travel&order=popular&per_page=7&pretty=true`),      
      axios.get(`${PIX_URL}key=${process.env.PIX_KEY}&q=${countryName}&image_type=photo&orientation=horizontal&category=travel&order=popular&per_page=7&pretty=true`),
    ]);
